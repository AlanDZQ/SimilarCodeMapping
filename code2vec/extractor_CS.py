...
class Extractor:
    def __init__(self, config, jar_path, max_path_length, max_path_width):
        ...
        self.csproj = 'CSharpExtractor/CSharpExtractor/Extractor/Extractor.csproj'
        self.ofile_name = 'test.raw.txt'

    def extract_paths(self, path):
        command = ['dotnet', 'run', '--project', str(self.csproj), '--path', path, '--ofile_name', str(self.ofile_name), '--no_hash']
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = process.communicate()
        output = out.decode().splitlines()

        # output = list(open('my_dataset.test.raw.txt', "r", encoding='utf-8').readlines())
        if len(output) == 0:
